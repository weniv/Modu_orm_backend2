...
* WBS 작성
    * 아래와 같은 서비스(간단한 요구사항 문서를 작성해 업로드)를 Django로 만들 예정이야. AWS에 배포하고, 프로젝트 시작일은 8월 20일이야. 이 프로젝트를 요구사항 분석하는 단계부터 WBS를 머메이드로 작성해줘.
* 요구사항 분석, 기능 명세 작성
* ERD 작성 => model 작성
* UI 디자인 => 프론트엔드 개발
* 백엔드 개발
...
git add .
git commit -m "account, tube 앱 생성"
...
git add .
git commit -m 'settings.py 수정 및 요구사항 모델 반영'

# settings.py 수정했을 때 한 번 commit 했었으면 더 좋았습니다.
###################################
...
asset 다운로드 후 썸네일과 영상 포함한 게시물 3개 업로드
...
git add .
git commit -m 'tube앱의 urls.py, views.py(list, detail, tag), forms.py 수정'

###################################
# tube > templates > tube > tube_list.html
# tube > templates > tube > tube_detail.html
...
###################################
# tube > tube_create.html, tube_update.html
...
# 수업에서는 tailwind css를 설치형을 쓰지 않고 cdn을 사용합니다.
# 사용법은 https://pypi.org/project/django-tailwindcss/ 를 참고하세요.
# 설치는 pip install django-tailwindcss 명령어를 사용할 수 있습니다.

# bootstrap은 돈주고 사는 용도로 자주 사용합니다. 디자이너가 디자인한 UI가 갖춰진 상태에서는 부트스트랩을 잘 사용하지 않습니다. Bootstrap은 Front-end 개발자의 만족도가 낮습니다. 백엔드 개발자가 프론트 엔드 개발자 없이 개발할 때에 만족도가 높은 편입니다.

# tailwind는 디자이너가 디자인한 UI가 없는 상태에서 개발할 때 사용하기 좋습니다. 커스터마이징 하기 매우 편리합니다.
